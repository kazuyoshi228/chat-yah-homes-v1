...
## 本体側への推奨（2026-08-18）：admin/properties は施設ごとにURLを分ける

`/admin/properties/kiyokawa/`・`/admin/properties/takasago/` のように施設別URLを推奨
（`/admin/properties/` は施設一覧の入口に）。データは施設ごとに別ドキュメントのため保存が
混じることはないが、1ページに複数施設のフォームが並ぶと**別施設を編集してしまう事故**が
起きやすく、施設数が増えるほどリスクが上がる。公開側 `/properties/{id}/` とも対称になり、
SSoTマップから貼る編集リンクも施設単位で確定する（アンカーよりページ分割が堅牢）。

セクションアンカーは**施設名を含めず共通名**にする：chat用情報は
`/admin/properties/kiyokawa/#chat`（現行の `#chat-kiyokawa` は施設別URL化と同時に `#chat` へ）。
全施設でリンク規則が統一され、肥大化したら `/admin/properties/{id}/chat/` へ昇格できる。
